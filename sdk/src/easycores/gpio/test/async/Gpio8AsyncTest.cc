 ...
 */
class Gpio8AsyncTest : public Tester
{
    std::string testName(void) {
        return "Async test functionality of gpio8";
    }

    bool testMethod(void) {
        gpio8_async_test_fpga_ptr fpga = std::make_shared<Gpio8AsyncTestFpga>();

        if (!fpga->init(0, "Gpio8AsyncTestFpga.bin")) {
            return false;
        }
        ...
        Log().Get(INFO) << "Number of requests: " << fpga->getCommunicator()->getNumberOfPendingAsyncRequests();
